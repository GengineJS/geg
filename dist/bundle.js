...
function updateListeners (
    ...
    for (name in on) {
        ...
        if (isUndef$1(cur)) ; else if (isUndef$1(old)) {
...
function initData (gm) {
  ...
  while (i--) {
    const key = keys[i];
    if (props && hasOwn$1(props, key)) ; else if (!isReserved(key)) {
      proxy(gm, `_data`, key);
    }
  }
  // observe data
...
function _createElement (
  ...
  if (typeof tag === 'string') {
    ...
    } else if ((!data || !data.pre) && isDef$1(Ctor = resolveAsset(context.$options, 'components', tag))) {
      // component
...
function initMixin$1 (Gengine) {
  Gengine.prototype._init = function (options) {
    const gm = this;
    // a uid
    gm._uid = uid$2++;
...
class Gengine extends BaseInterface {
    constructor(options) {
        super(options);
    }
}
...
function sameInputType (a, b) {
  ...
  let i;
  const typeA = isDef$1(i = a.data) && isDef$1(i = i.attrs) && i.type;
  const typeB = isDef$1(i = b.data) && isDef$1(i = i.attrs) && i.type;
...
function createPatchFunction (backend) {
  ...
  function createComponent (vnode, insertedVnodeQueue, parentElm, refElm) {
    let i = vnode.data;
    if (isDef$1(i)) {
      const isReactivated = isDef$1(vnode.componentInstance) && i.keepAlive;
...
const isNotTextNode = (c) => c.tag || isAsyncPlaceholder$1(c);
...
Gengine.prototype.__patch__ = inBrowser ? patch : noop;
...
var has$1 = function(object, propertyName) {
    return hasOwnProperty$1.call(object, propertyName);
};
...
var merge = function(options, defaults) {
    ...
    for (key in defaults) {
        ...
        result[key] = has$1(options, key) ? options[key] : defaults[key];
    }
    return result;
};
...
var codePointToSymbol = function(codePoint, strict) {
    ...
    if (has$1(decodeMapNumeric, codePoint)) {
        if (strict) {
...
var encode = function(string, options) {
    ...
    if (encodeEverything) {
        ...
        string = string.replace(regexAsciiWhitelist, function(symbol) {
            ...
            if (useNamedReferences && has$1(encodeMap, symbol)) {
        ...
        if (modifiers) {
          ...
          if (modifiers.sync) {
            ...
            if (!isDynamic) {
              ...
              if (hyphenate$1(name) !== camelize(name)) {
                addHandler(
                  el,
